...
void printChilds(struct FileNode **head, int *i, int siblingsLev[100], FILE *f)
{
    ...
    if (f != NULL)
    {
        for (int j = 1; j < *i; j++)
        {
            // 1. Linea verticale: sostituisci il tuo vecchio blocco con questo
            if (siblingsLev[j] == 1)
            {
                fprintf(f, "\xE2\x94\x82   "); // Questo scrive il carattere "│" in UTF-8 puro
            }
            else
            {
                fprintf(f, "    ");
        ...
        // Angolo finale e bivi
        if (curr->siblings == NULL)
        {
            fprintf(f, "\xE2\x94\x94\xE2\x94\x80\xE2\x94\x80 "); // Questo scrive "└── " in UTF-8 puro
        }
        else
        {
            fprintf(f, "\xE2\x94\x9C\xE2\x94\x80\xE2\x94\x80 "); // Questo scrive "├── " in UTF-8 puro
        }
        if (curr->type == 'D')
            fprintf(f, "%s/\n", curr->name);
        else
    ...
    else
    {
        ...
        if (curr->siblings == NULL)
            printf("\xC0\xC4\xC4 "); // Angolo: └──
        else
            printf("\xC3\xC4\xC4 "); // Bivio:  ├──
        if (curr->type == 'D')
            printf("%s/\n", curr->name);
        else
            printf("%s\n", curr->name);
